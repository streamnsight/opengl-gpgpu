...
vec3 mb(vec3 p) {
	...
	vec3 z = p;
	vec3 dz=vec3(0.0);
	float power = 8.0;   // higher values = more convoluted bulb
	float r, theta, phi;
	float dr = 1.0;
...
// 	uv.x*=iResolution.x/iResolution.y; //aspect ratio
    ...
    pixel_size = 1.0/(width * 3.0);
	// camera
 	stime=0.7+0.3*sin(scale*0.4);
 	ctime=0.7+0.3*cos(scale*0.4);
    ...
    vec3 cu = normalize(cross(cs,cf));
 	vec3 rd = normalize(uv.x*cs + uv.y*cu + (1.0 + scale * 0.1)*cf);  // transform from view to world
...
// halo
	vec3 bg = vec3(0.0, 0.0, 0.0); //exp(uv.y-5.0)*vec3(0.4, 1.6, 1.0);
